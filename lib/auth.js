...
export function verifyAdminToken(token) {
  if (!token) return false
  // Check in-memory (works within same instance)
  const valid = global._adminTokens || new Set()
  if (valid.has(token)) return true
  // Fallback: accept HMAC-signed token derived from credentials (survives cold starts)
  const expected = crypto.createHmac('sha256', process.env.ADMIN_PASS || 'lastnitro51')
    .update(process.env.ADMIN_EMAIL || 'lastnitro51@gmail.com')
    .digest('hex')
